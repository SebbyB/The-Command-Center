...
/**
 * Generic directory-listing component for virtual filesystem subdirectories.
 *
 * Reads directly from the navigation context's currentDir, so it works for any
 * depth without needing a static import of the section's data list.
 *
 * Children that have a pageMap entry are rendered as full page cards.
 * Children that don't are rendered as plain directory entries (subdirs).
 */
const SubdirList: React.FC = () => {
  const { currentPath, currentDir, onNavigate } = useNavigation();
  const pathStr = '~/' + currentPath.join('/');

  const dirEntries = Object.entries(currentDir.children).filter(
    ([, node]) => node.type === 'directory',
  );

  const subdirs = dirEntries.filter(([name]) => !pageMap[name]).map(([name]) => name);
  const pages   = dirEntries.filter(([name]) =>  pageMap[name]).map(([name]) => pageMap[name]!);

  const totalCount = subdirs.length + pages.length;

  return (
    <SectionLayout withBackground>
      <div style={{ marginBottom: '2rem' }}>
        <span className="terminal-cyan">{pathStr}</span>
