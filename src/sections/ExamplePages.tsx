...
import { examplePagesList } from '../filesystem/example-pages';
import { useNavigation } from '../context/navigation';
import { cardHoverHandlers } from '../utils/hoverHandlers';
import SectionLayout from '../components/SectionLayout';

const ExamplePages: React.FC = () => {
  const { currentPath, onNavigate } = useNavigation();

  return (
    <SectionLayout withBackground>
        <div style={{ marginBottom: '2rem' }}>
          <span className="terminal-cyan">~/example-pages</span>
          <span className="terminal-text-muted"> — {examplePagesList.length} entr{examplePagesList.length !== 1 ? 'ies' : 'y'}</span>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          {examplePagesList.map((entry) => (
            <div
              key={entry.name}
